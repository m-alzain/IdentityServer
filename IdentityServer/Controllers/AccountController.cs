...
namespace IdentityServer.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //    $"Please confirm your account by clicking this link: <a href='{callbackUrl}'>link</a>",model.Email);
                    await _emailSender.SendSmtpEmail(model.Email, "Confirm your account",
                        $"Please confirm your account by clicking this link: <a href='{callbackUrl}'>link</a>", model.Email);
                    //await _signInManager.SignInAsync(user, isPersistent: false);
                    _logger.LogInformation(3, "User created a new account with password.");
                    return RedirectToLocal(returnUrl); // return a comfirmation email has been sent to the accout (model.email)
                }
                AddErrors(result);
            }
        ...
        public async Task<IActionResult> ExternalLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            //else
            //{
            //    //get the email from the external login
            //    var email = info.Principal.FindFirstValue(ClaimTypes.Email);
            ...
            return await FinishExternalLogin(result, user, provider, providerUserId, returnUrl);
        ...
        public async Task<IActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl = null)
        {
            if (ModelState.IsValid)
            {
                ...
                //try
                //{
                //    var user = await ProvisionUserAsync(model.Provider, model.ProviderUserId, model.Email);                   
                ...
                var (user, provider, providerUserId, claims) = await FindUserFromExternalProviderAsync(result);
                user = await ProvisionUserAsync(provider, providerUserId, claims);
                return await FinishExternalLogin(result, user, provider, providerUserId, returnUrl,toSetPassword:true);
            }
            ViewData["ReturnUrl"] = returnUrl;
            return View(model);
        }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await _emailSender.SendSmtpEmail(model.Email, "Reset Password",
                   $"Please reset your password by clicking here: <a href='{callbackUrl}'>link</a>", model.Email);
                return View("ForgotPasswordConfirmation");
            }
        ...
        public async Task<IActionResult> SendCode(SendCodeViewModel model)
        {
            ...
            if (model.SelectedProvider == "Email")
            {
                ...
                await _emailSender.SendSmtpEmail(await _userManager.GetEmailAsync(user), "Security Code", message, "Hi Sir");
            }
        ...
        private async Task<ApplicationUser> ProvisionUserAsync(string provider, string providerUserId, IEnumerable<Claim> claims)
        {
            ...
            var user = new ApplicationUser
            {
                UserName = name.Split(' ')[0] ?? email ?? Guid.NewGuid().ToString(),
        ...
        //private async Task<ApplicationUser> ProvisionUserAsync(string provider, string providerUserId, string email)
        //{
        //    // create a list of claims that we want to transfer into our store
        ...
        private void ProcessLoginCallbackForOidc(AuthenticateResult externalResult, List<Claim> localClaims, Microsoft.AspNetCore.Authentication.AuthenticationProperties localSignInProps)
        {
        ...
        private void ProcessLoginCallbackForWsFed(AuthenticateResult externalResult, List<Claim> localClaims, Microsoft.AspNetCore.Authentication.AuthenticationProperties localSignInProps)
        {
        }

        private void ProcessLoginCallbackForSaml2p(AuthenticateResult externalResult, List<Claim> localClaims, Microsoft.AspNetCore.Authentication.AuthenticationProperties localSignInProps)
        {
        }

        private async Task<IActionResult> FinishExternalLogin(AuthenticateResult result, ApplicationUser user, string provider, string providerUserId, string returnUrl, bool toSetPassword = false)
        {
            // this allows us to collect any additonal claims or properties
            // for the specific prtotocols used and store them in the local auth cookie.
            // this is typically used to store data needed for signout from those protocols.
            var additionalLocalClaims = new List<Claim>();
            var localSignInProps = new Microsoft.AspNetCore.Authentication.AuthenticationProperties();
